...
const DEFAULT_GLOW_COLOR = "15, 83, 242"; // Oriana primary blue
...
const MagicBento = ({
  ...
  const cardClassName = `mb-magic-card ${textAutoHide ? "mb-text-autohide" : ""} ${enableBorderGlow ? "mb-border-glow" : ""}`;
    ...
    <>
      ...
      <div className="mb-card-grid mb-bento-section" ref={gridRef}>
        {cards.map((card, index) => {
          ...
          const content = (
            <>
              <div className="mb-card-header">
                <div className="mb-card-label">{card.number}</div>
              </div>
              <div className="mb-card-content">
                <h3 className="mb-card-title">{card.title}</h3>
                <p className="mb-card-description">{card.description}</p>
              </div>
            </>
          );
          ...
          if (enableStars) {
            return (
              <ParticleCard key={index} className={cardClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                {content}
              </ParticleCard>
            );
          }
